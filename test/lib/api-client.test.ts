...
describe("api-client", () => {
  ...
  describe("apiSend", () => {
    it("serializes the body and sets content-type", async () => {
      ...
      const result = await apiSend("/portfolio/weights", "POST", { target_weight: 0.3 })
      expect(fetchMock).toHaveBeenCalledWith(
    ...
    it("throws an ApiError on a failed mutation", async () => {
      ...
      await expect(apiSend("/strategies/1", "DELETE")).rejects.toBeInstanceOf(ApiError)
    })
  })
  ...
  describe("getWithFallback", () => {
    ...
    it("returns the fallback when the request fails", async () => {
      ...
      const res = await getWithFallback("/x", fallback)
      expect(res).toEqual({ data: fallback, isFallback: true })
    })
  })
})
